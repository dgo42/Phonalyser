...
final class SignalMeasurements {
    ...
    private SignalMeasurements(double vpp, double vrms, double vmean,
                               double period, double frequency, double dutyCycle) {
        this.vpp = vpp;
        this.vrms = vrms;
        this.vmean = vmean;
        this.period = period;
    ...
    static SignalMeasurements compute(float[] data, int n, double sampleRate, double peakVolts) {
        if (n < 4) {
            return new SignalMeasurements(0, 0, 0, Double.NaN, Double.NaN, Double.NaN);
        }
        double sum = 0;
        double sumSq = 0;
        float min = data[0];
        ...
        return new SignalMeasurements(vpp, vrms, vmean, period, frequency, duty);
    }
    ...
    private SignalMeasurements() { this(0, 0, 0, Double.NaN, Double.NaN, Double.NaN); }
}
