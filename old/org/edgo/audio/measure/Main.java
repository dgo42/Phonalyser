 ...
 *     [--backend csjsound|wdmks]   default: csjsound — uses csjsound-provider for
 *                                  WASAPI-exclusive bit-perfect access (24- and 32-bit).
 *                                  wdmks routes capture/playback through PortAudio's
 *                                  WDM-KS host API; needs portaudio_x64.dll on
 *                                  java.library.path.
 *
 *   ── List devices ─────────────────────────────────────────────────────────
 *     java -jar adc-calibration.jar --list
 *         Prints every input and output mixer the active backend exposes,
 *         with index, name, vendor, and supported PCM formats.
 *
 *   ── Histogram capture ───────────────────────────────────────────────────
 *     java -jar adc-calibration.jar --histogram --samplerate <hz> --duration <s>
 ...
 *                                      CsjsoundRecorder.adcFsVoltageRms as 2·√2·V_rms
 ...
 *     java -jar adc-calibration.jar --histogram --load <file> --bits 8|16|24
 *         --window <n> --width <px> --height <px>
 ...
 *                                      2·√2·CsjsoundRecorder.adcFsVoltageRms)
 *         [--adc-fs-vrms <vrms>]       ADC full-scale RMS voltage
 *
 *   ── Analyze histogram (DNL + INL from a captured histogram CSV) ─────────
 *     java -jar adc-calibration.jar --analyze-histogram <file> --bits 8|16|24
 ...
 *     java -jar adc-calibration.jar --fft-analyze <wav_file> --fft-size <n>
 *         --width <px> --height <px>
 ...
 *                                      (defaults to CsjsoundRecorder.adcFsVoltageRms)
 ...
 *
 *   ── Record raw WAV ──────────────────────────────────────────────────────
 *     java -jar adc-calibration.jar --record-wav --samplerate <hz> --duration <s>
 ...
 *     java -jar adc-calibration.jar --record-mapped-wav <weights.csv> --samplerate <hz>
 ...
 *     java -jar adc-calibration.jar --process-wav <file> --load-weighted <weights.csv>
 ...
 *     java -jar adc-calibration.jar --regress-calibrate <wav_file> --scale <V>
 *         [--width <px>] [--height <px>]
 ...
 *     java -jar adc-calibration.jar --deembed --comp <csv> [--comp <csv> …]
 ...
 *     java -jar adc-calibration.jar --generate --samplerate <hz> --signal <form>
 ...
 *         --signal <form>              sine | sawtooth | triangle | white_noise |
 ...
 *     java -jar adc-calibration.jar --gen-fft --samplerate <hz> --freq <hz>
 ...
 *     java -jar adc-calibration.jar --iterative-compensate --samplerate <hz>
 ...
 *     java -jar adc-calibration.jar --freq-response --samplerate <hz>
...
public class Main {
    ...
    public void run(String[] args) throws Exception {
        ...
        // Resolve audio backend before any device lookup. csjsound stays the default.
    ...
    private void runHistogramMode(String[] args) throws Exception {
        ...
        if (adcFsArg != null) {
            CsjsoundRecorder.adcFsVoltageRms = Double.parseDouble(adcFsArg);
        }
        ...
        // Default: derive from CsjsoundRecorder.adcFsVoltageRms — for an FS-RMS rating
        // of 1.7931 V the peak-to-peak range is 2·√2·V_rms ≈ 5.072 V.
        double scaleVolts = scaleArg != null
                ? Double.parseDouble(scaleArg)
                : 2.0 * Math.sqrt(2.0) * CsjsoundRecorder.adcFsVoltageRms;
        if (scaleVolts <= 0) {
    ...
    private void printUsage() {
        ...
        log.info("    [--backend csjsound|wdmks]");
        log.info("                              default csjsound (WASAPI exclusive via csjsound-provider)");
        log.info("                              wdmks routes capture/playback through PortAudio's WDM-KS");
        log.info("                              host API; requires portaudio_x64.dll on java.library.path");
        log.info("");
        ...
        log.info("                              absent; defaults to CsjsoundRecorder.adcFsVoltageRms)");
        ...
        log.info("    --signal <form>           sine | sawtooth | triangle | white_noise |");
        ...
        log.info("    [--signal <form>]         sine | sawtooth | triangle | *_noise |");
    ...
    private void logProviders() {
        ...
        boolean csjFound = false;
        for (MixerProvider p : ServiceLoader.load(MixerProvider.class)) {
            String name = p.getClass().getName();
            log.info("  {}", name);
            if (name.toLowerCase().contains("csj")) {
                csjFound = true;
            }
        }
        if (!csjFound) {
            for (Mixer.Info mi : AudioSystem.getMixerInfo()) {
                String v = (mi.getVendor() + " " + mi.getName() + " " + mi.getDescription()).toLowerCase();
                if (v.contains("csj") || v.contains("wasapi")) {
                    csjFound = true;
                    break;
                }
            }
        ...
        if (csjFound) {
            log.info("csjsound-provider detected.");
        } else {
            log.warn("csjsound-provider NOT found — running on default Java Sound driver (max 16-bit).");
            log.warn("Ensure csjsound-provider.jar is on the classpath and csjsound_amd64.dll is on java.library.path.");
        }
    }
    ...
    private static void saveFilterCalibrationCsv(FilterCalibration cal, String path,
        ...
        try (PrintWriter pw = new PrintWriter(
            ...
            pw.printf(Locale.US, "# adc_fs_voltage_rms=%.6f%n", CsjsoundRecorder.adcFsVoltageRms);
            ...
            double dbvScale = CsjsoundRecorder.adcFsVoltageRms;
    ...
    private static void applyDefaultDbvScaling(FftAnalyzer.Result result) {
        if (Double.isNaN(result.fundRefDbV) && CsjsoundRecorder.adcFsVoltageRms > 0.0) {
            result.fundRefDbV = result.fundamentalDbFs
                    + 20.0 * Math.log10(CsjsoundRecorder.adcFsVoltageRms);
        }
    }
