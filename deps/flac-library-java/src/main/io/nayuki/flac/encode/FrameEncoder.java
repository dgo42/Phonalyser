...
final class FrameEncoder {
	...
	public static SizeEstimate<FrameEncoder> computeBest(int sampleOffset, long[][] samples, int sampleDepth, int sampleRate, SubframeEncoder.SearchOptions opt) {
		...
		} else if (sampleDepth + 1 > 32) {
			// Side-coded modes need a (sampleDepth + 1)-bit side channel,
			// which overflows the decoder's readUint / readSignedInt
			// 32-bit limit for 32-bit input.  Force INDEPENDENT
			// (mode 1 = left/right) so the file round-trips through
			// this same library.
			long[] left  = samples[0];
